...
create unique index if not exists reading_room_one_active_user_per_slot
...
create or replace function public.reserve_reading_room_seat(
  target_date date,
  target_time_slot text,
  target_seat_number integer
)
returns uuid
language plpgsql
security definer
...
begin
  ...
  if target_seat_number not between 1 and 24 then
    raise exception '올바른 좌석을 선택해 주세요';
  end if;
  if target_date < current_korea_date or target_date > current_korea_date + 14 then
    raise exception '열람실은 오늘부터 14일 이내 날짜만 예약할 수 있습니다';
  end if;
  ...
  if exists (
    ...
    raise exception '같은 시간에는 좌석을 하나만 예약할 수 있습니다';
  end if;

  insert into public.reading_room_reservations (
    user_id, seat_number, reservation_date, time_slot
  ) values (
    current_user_id, target_seat_number, target_date, target_time_slot
  )
  returning id into new_reservation_id;
...
revoke all on function public.reserve_reading_room_seat(date, text, integer) from public, anon;
grant execute on function public.reserve_reading_room_seat(date, text, integer) to authenticated;
